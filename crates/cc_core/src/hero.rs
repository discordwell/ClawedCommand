...
mod tests {
    ...
    fn hero_modifiers_are_non_negative() {
        for hero in ALL_HEROES {
            ...
            assert!(
                mods.health_bonus >= Fixed::ZERO,
                "{hero:?} health_bonus negative"
            );
            assert!(
                mods.speed_multiplier > Fixed::ZERO,
                "{hero:?} speed_multiplier non-positive"
            );
            assert!(
                mods.damage_bonus >= Fixed::ZERO,
                "{hero:?} damage_bonus negative"
            );
            assert!(
                mods.range_bonus >= Fixed::ZERO,
                "{hero:?} range_bonus negative"
            );
        }
    }
    ...
    fn all_heroes_have_factions() {
        for hero in ALL_HEROES {
            assert!(
                !hero_faction(hero).as_str().is_empty(),
                "{hero:?} has empty faction"
            );
        }
    }
    ...
    fn hero_data_struct_matches_convenience_fns() {
        for hero in ALL_HEROES {
            ...
            assert_eq!(
                data.base_kind,
                hero_base_kind(hero),
                "{hero:?} base_kind mismatch"
            );
            assert_eq!(data.name, hero_name(hero), "{hero:?} name mismatch");
            assert_eq!(
                data.faction,
                hero_faction(hero),
                "{hero:?} faction mismatch"
            );
            // Also verify the string form matches
            assert_eq!(
                data.faction.as_str(),
                hero_faction_str(hero),
                "{hero:?} faction_str mismatch"
            );
            let mods = hero_modifiers(hero);
            assert_eq!(
                data.modifiers.health_bonus, mods.health_bonus,
                "{hero:?} health_bonus mismatch"
            );
            assert_eq!(
                data.modifiers.speed_multiplier, mods.speed_multiplier,
                "{hero:?} speed_multiplier mismatch"
            );
            assert_eq!(
                data.modifiers.damage_bonus, mods.damage_bonus,
                "{hero:?} damage_bonus mismatch"
            );
            assert_eq!(
                data.modifiers.range_bonus, mods.range_bonus,
                "{hero:?} range_bonus mismatch"
            );
        }
    }
}
