...
mod tests {
    ...
    fn path_prefers_road_over_grass() {
        ...
        for pos in &path {
            assert_eq!(
                pos.y, 0,
                "Path should stay on road row, but visited {:?}",
                pos
            );
        }
    }
    ...
    fn path_uses_ford_to_cross_river() {
        ...
        assert!(
            path.contains(&GridPos::new(5, 5)),
            "Path should use the ford"
        );
    ...
    fn elevation_blocks_without_ramp() {
        ...
        assert!(
            path.is_none(),
            "Should not path across elevation without ramp"
        );
    }
