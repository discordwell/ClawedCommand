    ...
    match name {
        ...
        "get_units" => {
            let Some(snap) = snapshot else {
                return (
                    serde_json::json!({"error": "no game state available"}),
                    vec![],
                );
            };
        ...
        "get_buildings" => {
            ...
            (
                serde_json::json!({"buildings": buildings, "count": count}),
                vec![],
            )
        }
        "get_visible_enemies" => {
            ...
            (
                serde_json::json!({"enemies": enemies, "count": count}),
                vec![],
            )
        }
        "get_resources" => {
        ...
        "move_units" => {
            ...
            let Some(x) = args["x"].as_i64() else {
                return (
                    serde_json::json!({"error": "missing required field: x"}),
                    vec![],
                );
            };
            let Some(y) = args["y"].as_i64() else {
                return (
                    serde_json::json!({"error": "missing required field: y"}),
                    vec![],
                );
            };
            (
                serde_json::json!({"status": "ok"}),
                vec![GameCommand::Move {
                    unit_ids,
                    target: GridPos::new(x as i32, y as i32),
                }],
            )
        }
        "attack_units" => {
            ...
            let Some(target_id) = args["target_id"].as_u64() else {
                return (
                    serde_json::json!({"error": "missing required field: target_id"}),
                    vec![],
                );
            };
            (
                serde_json::json!({"status": "ok"}),
                vec![GameCommand::Attack {
                    unit_ids,
        ...
        "build" => {
            let Some(builder_id) = args["builder_id"].as_u64() else {
                return (
                    serde_json::json!({"error": "missing required field: builder_id"}),
                    vec![],
                );
            };
            let Some(building_type) = args["building_type"].as_str() else {
                return (
                    serde_json::json!({"error": "missing required field: building_type"}),
                    vec![],
                );
            };
            let Some(kind) = building_type.parse::<BuildingKind>().ok() else {
                return (
                    serde_json::json!({"error": format!("unknown building type: {building_type}")}),
                    vec![],
                );
            };
            let Some(x) = args["x"].as_i64() else {
        ...
        "train_unit" => {
            let Some(building_id) = args["building_id"].as_u64() else {
                return (
                    serde_json::json!({"error": "missing required field: building_id"}),
                    vec![],
                );
            };
            let Some(unit_type) = args["unit_type"].as_str() else {
                return (
                    serde_json::json!({"error": "missing required field: unit_type"}),
                    vec![],
                );
            };
            let Some(kind) = unit_type.parse::<UnitKind>().ok() else {
                return (
                    serde_json::json!({"error": format!("unknown unit type: {unit_type}")}),
                    vec![],
                );
            };
            (
                serde_json::json!({"status": "ok"}),
                vec![GameCommand::TrainUnit {
        ...
        "gather_resource" => {
            ...
            let Some(deposit_id) = args["deposit_id"].as_u64() else {
                return (
                    serde_json::json!({"error": "missing required field: deposit_id"}),
                    vec![],
                );
            };
            (
                serde_json::json!({"status": "ok"}),
                vec![GameCommand::GatherResource {
                    unit_ids,
                    deposit: EntityId(deposit_id),
                }],
            )
        }
        "patrol" => {
        ...
        "hold_position" => {
            ...
            (
                serde_json::json!({"status": "ok"}),
                vec![GameCommand::HoldPosition { unit_ids }],
            )
        }
        // -----------------------------------------------------------------
        ...
        | "coordinate_assault"
        | "research_priority"
        | "adaptive_defense" => execute_behavior(name, args, player_id, snapshot),
...
fn execute_behavior(
    ...
    let result = match name {
        ...
        "assign_idle_workers" => behaviors::assign_idle_workers(&mut ctx),
        "attack_move_group" => {
            let ids = parse_entity_ids(args, "unit_ids");
            let x = args["x"].as_i64().unwrap_or(0) as i32;
            let y = args["y"].as_i64().unwrap_or(0) as i32;
        ...
        "protect_unit" => {
            ...
            behaviors::protect_unit(
                &mut ctx,
                &escort_ids,
                EntityId(vip_id),
                Fixed::from_num(guard_radius),
            )
        }
        "surround_target" => {
            let ids = parse_entity_ids(args, "unit_ids");
            ...
            behaviors::surround_target(
                &mut ctx,
                &ids,
                EntityId(target_id),
                Fixed::from_num(ring_radius),
            )
        }
        "balanced_production" => {
            let building_id = args["building_id"].as_u64().unwrap_or(0);
