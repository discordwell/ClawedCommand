-- @name: combat_micro_threat_kite
-- @events: on_tick
-- @interval: 3

-- Gen 47: Gen 42 base + per-unit threats_to() kite trigger.
-- Instead of only kiting when globally outnumbered, each ranged unit kites
-- when it personally has 2+ threats (enemies targeting it). More precise micro
-- that protects ranged units even when the army as a whole isn't outnumbered.
-- Budget: ~30/500 (6%) — threats_to costs 2 per ranged unit.
...
-- === PER-UNIT THREAT-BASED KITING (new vs Gen 42) ===
-- Each ranged unit checks its own threats instead of global outnumbered check
if enemies and #ranged_attackers > 0 then
    for _, r in ipairs(ranged_attackers) do
        ...
        if threats and #threats >= 2 then
            should_kite = true
        elseif outnumbered then
            -- Fallback: still kite if globally outnumbered
            should_kite = true
        end
