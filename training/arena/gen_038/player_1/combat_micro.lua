-- @name: combat_micro_hybrid
-- @events: on_tick
-- @interval: 3

-- Gen 38: Gen 34 (closest-enemy focus fire) + late-game aggression.
-- Combines the two best ideas: closest-enemy targeting (Gen 34, 80% wins)
-- and time-based push escalation from Gen 35 (but only the late-game
-- part, since mid-game escalation didn't help).
-- After tick 4500, disable retreat, disable kiting, push with any army,
-- and prioritize HQ over production.
...
if should_push then
    ...
    if enemy_buildings and #enemy_buildings > 0 then
        ...
        -- In late game, go straight for HQ
        local target
        if late_game then
