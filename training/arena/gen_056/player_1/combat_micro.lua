-- @name: combat_micro_kill_secure
-- @events: on_tick
-- @interval: 3

-- Gen 56: Gen 34 base + kill-secure targeting.
-- Within range 12, target enemy with lowest absolute HP (not ratio).
-- Maximizes confirmed kills to reduce enemy army size faster.
-- Falls back to closest-to-centroid if no enemy within range.
...
-- === KILL-SECURE TARGETING: lowest absolute HP within range ===
if #attackers >= 2 and enemies and #enemies > 0 then
    ...
    for _, e in ipairs(enemies) do
        ...
        local dy = e.y - cy
        local d = dx * dx + dy * dy
        if d < 12 * 12 and e.hp < lowest_hp then
            lowest_hp = e.hp
            best_target = e
        end
    end

    -- Fallback to closest if none in range
