-- @name: combat_micro_timed_push
-- @events: on_tick
-- @interval: 3

-- Gen 60: Gen 34 base + timed push at tick 4000.
-- Force aggression in final third regardless of advantage.
-- Targets timeout seeds (2, 4) where P1 has advantage but can't close.
...
-- === RETREAT (disabled in late game — commit mode) ===
...
-- === PUSH: normal logic OR forced push in late game ===
local should_push = (enemy_count == 0 and my_combat_count >= 2)
    or strong_advantage
    or late_game  -- FORCED PUSH after tick 4000
