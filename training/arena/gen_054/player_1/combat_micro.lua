-- @name: combat_micro_dps_weighted
-- @events: on_tick
-- @interval: 3

-- Gen 54: Gen 34 base + DPS-weighted targeting.
-- Score enemies by damage/hp — kill highest-DPS enemies first to reduce
-- incoming damage fastest. Uses unit stats for damage lookup.
...
-- === DPS-WEIGHTED TARGETING: highest damage/hp ratio near centroid ===
if #attackers >= 2 and enemies and #enemies > 0 then
    ...
    -- Known damage values per unit kind (approximate from base_stats)
    local unit_damage = {
        Nuisance = 8, Hisser = 12, Yowler = 6, FlyingFox = 10,
        Chonk = 15, Mouser = 20, Catnapper = 25, FerretSapper = 30,
        MechCommander = 18,
        -- Enemy faction equivalents (approximate)
        Scrapper = 8, Spitter = 12, Howler = 6, GliderRat = 10,
        Brute = 15, Shade = 20, Wrecker = 25, Blaster = 30,
    }
    ...
    for _, e in ipairs(enemies) do
        ...
        local d = dx * dx + dy * dy
        if d < 12 * 12 then
            -- Score = damage / current_hp (kill high-DPS low-HP first)
            local dmg = unit_damage[e.kind] or 10
            local score = dmg / math.max(e.hp, 1)
            -- Bias toward closer enemies (divide by distance factor)
            score = score / (1 + d / 100)
