-- @name: combat_micro_v2
-- @events: on_tick
-- @interval: 3

-- Gen 24: Improved combat micro for disadvantaged player.
-- Changes from Gen 21:
-- 1. Retreat at 30% HP (was 25%) — more conservative, preserves army better
-- 2. Rally toward own buildings (not home corner) for faster reinforcement
-- 3. Push targets enemy HQ specifically (ends games faster)
-- 4. Ranged units kite backward when enemies are too close
...
for _, u in ipairs(my_units) do
    ...
    if not is_worker then
        ...
        if u.attacking then
            ...
            -- Ranged units: Hisser, Yowler, FlyingFox, Catnapper
...
-- Find rally point: nearest own building (reinforcement point)
...
-- === KITE: ranged units move back if melee enemies too close ===
if enemies and #ranged_attackers > 0 then
    for _, r in ipairs(ranged_attackers) do
        ...
        -- If nearest enemy is within 2 tiles (melee range), kite backward
        if closest_dist < 5 then  -- sqrt(5) ~ 2.2 tiles
...
-- === PUSH: when no enemies visible, attack-move toward enemy HQ ===
if enemy_count == 0 and my_combat_count >= 3 then
    ...
    if enemy_buildings and #enemy_buildings > 0 then
        -- Prioritize HQ (TheBox, TheDumpster, TheGrotto, etc.)
        local hq = nil
        local nearest = nil
        local nearest_dist = 999999
        for _, b in ipairs(enemy_buildings) do
            -- HQ buildings are typically the first building type
