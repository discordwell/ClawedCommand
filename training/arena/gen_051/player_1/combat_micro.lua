-- @name: combat_micro_weakest_near
-- @events: on_tick
-- @interval: 3

-- Gen 48: Gen 42 base + hybrid targeting (closest cluster, then weakest).
-- Among enemies within 8 tiles of centroid, target the one with lowest HP.
-- Finishes off wounded enemies faster for attrition advantage.
-- Budget: ~15/500 (3%).
...
-- === HYBRID TARGETING: weakest enemy within close range of centroid ===
-- (new vs Gen 42's pure closest-to-centroid)
if #attackers >= 2 and enemies and #enemies > 0 then
    ...
    -- First: filter enemies within 8 tiles of centroid (close enough to engage)
    ...
    for _, e in ipairs(enemies) do
        ...
        local d = dx * dx + dy * dy
        if d < 8 * 8 then
            table.insert(nearby_enemies, e)
        end
    end

    local best_target = nil
    if #nearby_enemies > 0 then
        -- Among nearby enemies, pick the one with lowest HP (finish it off)
        ...
        -- No enemies nearby — fall back to pure closest (Gen 34 behavior)
...
-- === TERRAIN-AWARE KITING (same as Gen 42) ===
