-- @name: combat_micro_hold_idle_noretreat
-- @events: on_tick
-- @interval: 3

-- Gen 95: Anti-hold-position champion. 79% P1 win rate vs Gen 072 (80 seeds).
-- Overall effective win rate: 59% vs Gen 072 (both sides combined).
-- Three key innovations over Gen 072:
-- 1. Idle-only push: only non-attacking units attack_move to buildings,
--    preserving focus fire on engaged units (attack_move overrides focus fire)
-- 2. Push at tick 3600 instead of 4000: catches opponent still in formation
-- 3. No retreat: keeps wounded units fighting instead of fleeing to rally
...
local late_game = current_tick >= 3600
...
if should_push then
    ...
    if enemy_buildings and #enemy_buildings > 0 then
        ...
        if target then
            -- Only send idle (non-attacking) units to avoid overriding focus fire
            local idle_ids = {}
            for _, u in ipairs(combat_units) do
