-- @name: combat_micro_range_kite
-- @events: on_tick
-- @interval: 3

-- Gen 45: Gen 42 base + position_at_range kiting.
-- Instead of raw flee direction, use ctx:position_at_range() to find the
-- optimal max-range attack position. Maintains DPS while kiting.
-- Budget: ~25/500 (5%).
...
-- === POSITION_AT_RANGE KITING (new vs Gen 42) ===
-- Use API to find optimal max-range position instead of raw flee direction
if outnumbered and enemies and #ranged_attackers > 0 then
    for _, r in ipairs(ranged_attackers) do
        ...
        if closest_dist < 5 and closest_e then
            -- Use position_at_range to find optimal kite position at max range
            local kx, ky = ctx:position_at_range(
                r.x, r.y, closest_e.x, closest_e.y,
                math.floor(r.range)
            )
            if kx then
                ...
                -- position_at_range returned nil, use terrain-aware flee
                local flee_dx = r.x - closest_e.x
                local flee_dy = r.y - closest_e.y
                local mag = math.sqrt(flee_dx * flee_dx + flee_dy * flee_dy)
                if mag > 0 then
