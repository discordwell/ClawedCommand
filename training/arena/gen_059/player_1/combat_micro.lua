-- @name: combat_micro_hp_kite
-- @events: on_tick
-- @interval: 3

-- Gen 59: Gen 34 base + HP-threshold kiting.
-- Ranged units kite when their own HP < 70% (not global outnumber check).
-- Protects injured ranged units regardless of army composition.
-- Healthy ranged units keep fighting instead of kiting.
...
-- === HP-THRESHOLD KITING (new) ===
-- Each ranged unit kites when its own HP < 70%, regardless of army count
if enemies and #ranged_attackers > 0 then
    for _, r in ipairs(ranged_attackers) do
        local hp_pct = r.hp / math.max(r.hp_max, 1)
        local should_kite = hp_pct < 0.70

        -- Also kite if globally outnumbered (Gen 34 fallback)
        if not should_kite and outnumbered then
            should_kite = true
        end
