-- @name: combat_micro_staged_push
-- @events: on_tick
-- @interval: 3

-- Gen 61: Gen 34 base + staged push.
-- During push: melee vanguard moves 4 tiles ahead toward target,
-- ranged hold position until melee engages (within 6 tiles of enemy).
-- Prevents ranged units from walking into melee range during push.
...
for _, u in ipairs(my_units) do
    ...
    if not is_worker then
        ...
        if is_ranged then
            table.insert(ranged_combat, u)
        else
            table.insert(melee_combat, u)
        end
        if u.attacking then
...
if should_push then
    ...
    if enemy_buildings and #enemy_buildings > 0 then
        ...
        if target then
            -- Check if melee is near enemy (within 6 tiles)
            local melee_near_enemy = false
            if enemies and #enemies > 0 then
                for _, m in ipairs(melee_combat) do
                    for _, e in ipairs(enemies) do
                        local dx = m.x - e.x
                        local dy = m.y - e.y
                        if dx * dx + dy * dy < 6 * 6 then
                            melee_near_enemy = true
                            break
                        end
                    end
                    if melee_near_enemy then break end
                end
            else
                melee_near_enemy = true  -- No enemies left, everyone push
            end

            -- Melee: always attack_move to target
            local melee_ids = {}
            for _, m in ipairs(melee_combat) do
                table.insert(melee_ids, m.id)
            end
            if #melee_ids > 0 then
                ctx:attack_move(melee_ids, target.x, target.y)
            end

            -- Ranged: attack_move only if melee is engaged, otherwise hold
            if melee_near_enemy then
                local ranged_ids = {}
                for _, r in ipairs(ranged_combat) do
                    table.insert(ranged_ids, r.id)
                end
                if #ranged_ids > 0 then
                    ctx:attack_move(ranged_ids, target.x, target.y)
                end
            end
        end
    end
