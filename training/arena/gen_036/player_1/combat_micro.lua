-- @name: combat_micro_wider_kite
-- @events: on_tick
-- @interval: 3

-- Gen 36: Gen 30 base + wider kite range (sqrt(9)=3 tiles instead of sqrt(5)~2.2).
-- Ranged units flee when enemy within 3 tiles instead of 2.2. More breathing room
-- should keep ranged units alive longer, especially Hissers and Yowlers.
...
-- === KITE when outnumbered — wider range (9 = 3^2 tiles) ===
if outnumbered and enemies and #ranged_attackers > 0 then
    for _, r in ipairs(ranged_attackers) do
        ...
        if closest_dist < 9 then  -- 3 tiles instead of sqrt(5)~2.2
