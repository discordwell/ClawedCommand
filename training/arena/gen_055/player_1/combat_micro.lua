-- @name: combat_micro_split_roles
-- @events: on_tick
-- @interval: 3

-- Gen 55: Gen 34 base + two-target split targeting.
-- Melee focus closest enemy (front line engagement).
-- Ranged focus weakest enemy within their range (snipe kills).
-- Natural role specialization without scattering the army.
    ...
    if not is_worker then
        ...
        if u.attacking then
            ...
            if is_ranged then
                ...
                table.insert(melee_attackers, u)
            end
        end
    end
end
...
if enemies and #enemies > 0 then
    -- MELEE: focus closest enemy to melee centroid
    if #melee_attackers >= 1 then
        local mcx, mcy = 0, 0
        for _, u in ipairs(melee_attackers) do
            mcx = mcx + u.x
            mcy = mcy + u.y
        end
        mcx = mcx / #melee_attackers
        mcy = mcy / #melee_attackers
    ...
    -- RANGED: focus weakest enemy within range 10 of ranged centroid
    if #ranged_attackers >= 1 then
        local rcx, rcy = 0, 0
        for _, u in ipairs(ranged_attackers) do
            rcx = rcx + u.x
            rcy = rcy + u.y
        end
        rcx = rcx / #ranged_attackers
        rcy = rcy / #ranged_attackers
        ...
        for _, e in ipairs(enemies) do
            ...
            local dy = e.y - rcy
            local d = dx * dx + dy * dy
            if d < 10 * 10 and e.hp < lowest_hp then
                lowest_hp = e.hp
                best_target = e
            end
        end
