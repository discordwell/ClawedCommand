-- @name: combat_micro_terrain_aware
-- @events: on_tick
-- @interval: 3

-- Gen 41: Gen 34 base + full terrain awareness package.
-- Adds: terrain-aware kiting, elevation positioning, cover-seeking retreat,
-- elevation-aware targeting. Budget ~70/500 (14%).
...
-- === RETREAT with COVER-SEEKING (2c) ===
local retreat_ids = {}
for _, u in ipairs(combat_units) do
    local hp_pct = u.hp / math.max(u.hp_max, 1)
    if hp_pct < 0.30 and u.attacking and outnumbered then
        -- Try to find a covered safe position instead of just rallying
        local safe = ctx:safe_positions(u.id, 5)
        local cover_pos = nil
        if safe then
            for _, sp in ipairs(safe) do
                local cover = ctx:cover_at(sp.x, sp.y)
                if cover ~= "None" then
                    cover_pos = sp
                    break
                end
        ...
        if cover_pos then
            ctx:move_units({u.id}, cover_pos.x, cover_pos.y)
        else
...
if #attackers >= 2 and enemies and #enemies > 0 then
    ...
    -- Score enemies: prefer close + lower elevation
...
if outnumbered and enemies and #ranged_attackers > 0 then
    for _, r in ipairs(ranged_attackers) do
        ...
        if closest_dist < 5 then
            ...
            -- Normalize to ~3 tile flee distance
            ...
            if cost == nil or cost >= 1.3 then
                -- Primary blocked or slow — try perpendicular directions
                ...
                -- If both perpendiculars also bad, use original (pathfinder will handle)
            end
...
-- === ELEVATION POSITIONING when idle (2b) ===
-- When not in combat, move idle units to nearby high ground
if enemy_count == 0 or (not outnumbered and #attackers == 0) then
    for _, u in ipairs(combat_units) do
        if not u.attacking then
            local my_elev = ctx:elevation_at(u.x, u.y)
            local best_elev = my_elev
            local best_ex, best_ey = u.x, u.y
            -- Scan 3-tile radius for higher ground
            for dx = -3, 3 do
                for dy = -3, 3 do
                    if dx ~= 0 or dy ~= 0 then
                        ...
                        if tx >= 0 and tx < map_w and ty >= 0 and ty < map_h then
                            ...
                            if elev > best_elev then
                                local passable = ctx:is_passable(tx, ty)
                                if passable then
                                    best_elev = elev
                                    best_ex = tx
                                    best_ey = ty
            ...
            if best_elev > my_elev then
                ctx:move_units({u.id}, best_ex, best_ey)
            end
        end
    end
end
