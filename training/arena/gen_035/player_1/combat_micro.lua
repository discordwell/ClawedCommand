-- @name: combat_micro_late_aggro
-- @events: on_tick
-- @interval: 3

-- Gen 35: Gen 30 base + time-based aggression escalation.
-- After tick 3000 (half the 6000 tick limit), lower push threshold from
-- >=3 advantage to >=1 advantage. After tick 4500 (75%), push with ANY
-- army regardless of advantage. This converts P1-leading timeouts to wins
-- by forcing the closing push before time runs out.
...
-- Time-based aggression escalation
local mid_game = tick >= 3000
local late_game = tick >= 4500

-- === RETREAT (disabled in late game — commit fully) ===
...
-- === PUSH: time-escalating aggression ===
local should_push
if late_game then
    -- After tick 4500: push with any army size
    should_push = my_combat_count >= 1
elseif mid_game then
    -- After tick 3000: push with any advantage
    should_push = (enemy_count == 0 and my_combat_count >= 2)
        or (my_combat_count > enemy_count)
else
    -- Early game: standard push threshold
    should_push = (enemy_count == 0 and my_combat_count >= 2)
        or strong_advantage
end
...
if should_push then
    ...
    if enemy_buildings and #enemy_buildings > 0 then
        ...
        -- In late game, prioritize HQ over production (go for the kill)
        local target
        if late_game then
            target = hq or prod_target or nearest
        else
            target = prod_target or hq or nearest
        end
        if target then
