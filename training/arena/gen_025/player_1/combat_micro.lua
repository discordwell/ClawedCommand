-- @name: combat_micro_v3
-- @events: on_tick
-- @interval: 3

-- Gen 25: Aggressive combat micro — convert timeouts to wins.
-- Changes from Gen 24:
-- 1. Removed kiting (caused stalemates, too many timeouts)
-- 2. Push when army advantage (not just when 0 enemies visible)
-- 3. Lower push threshold from 3 to 2 units
-- 4. Keep 30% retreat and building rally from Gen 24
...
-- Find rally point: nearest own building
...
local has_advantage = my_combat_count > enemy_count + 1
...
-- === PUSH: attack-move toward enemy HQ when advantaged or area clear ===
local should_push = (enemy_count == 0 and my_combat_count >= 2)
    or (has_advantage and my_combat_count >= 4)
