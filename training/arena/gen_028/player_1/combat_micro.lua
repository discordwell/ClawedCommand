-- @name: combat_micro_v6
-- @events: on_tick
-- @interval: 3

-- Gen 28: Gen 26 base + base defense recall.
-- When enemies detected near own buildings, rally all combat units to defend.
-- This should prevent losing to early rushes and close base attacks.
...
-- Find own buildings and rally point
...
-- === BASE DEFENSE: detect enemies near our buildings ===
local base_threatened = false
local threat_x, threat_y = 0, 0
local threat_count = 0
if enemies and my_buildings then
    for _, e in ipairs(enemies) do
        for _, b in ipairs(my_buildings) do
            local dx = e.x - b.x
            local dy = e.y - b.y
            if dx * dx + dy * dy < 64 then  -- within 8 tiles
                base_threatened = true
                threat_x = threat_x + e.x
                threat_y = threat_y + e.y
...
if base_threatened and threat_count > 0 then
    threat_x = threat_x / threat_count
    threat_y = threat_y / threat_count

    -- Rally all combat to the threat centroid via attack-move
    ctx:attack_move(all_combat_ids, math.floor(threat_x), math.floor(threat_y))

    -- Still do focus fire on the weakest enemy near the threat
    local weak = ctx:weakest_enemy_in_range(threat_x, threat_y, 12)
    if weak then
        ctx:attack_units(all_combat_ids, weak.id)
    end
    return  -- base defense overrides all other behaviors
end
...
-- === KITE: ranged units flee when outnumbered ===
