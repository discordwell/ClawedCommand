-- @name: combat_micro_closest_focus
-- @events: on_tick
-- @interval: 3

-- Gen 34: Gen 30 base + closest-enemy focus fire (instead of weakest).
-- Hypothesis: targeting the closest enemy to centroid kills the immediate
-- threat faster, preventing ranged units from being picked off. Weakest
-- targeting can cause the army to chase a distant low-HP unit while a
-- nearby full-HP unit deals damage unopposed.
...
-- === FOCUS FIRE: closest enemy to centroid instead of weakest ===
if #attackers >= 2 and enemies and #enemies > 0 then
    ...
    -- Find closest enemy to attacker centroid within range 12
    local best_target = nil
    local best_dist = 12 * 12  -- range 12 squared
