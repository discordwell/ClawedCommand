-- @name: combat_micro_push_lower
-- @events: on_tick
-- @interval: 3

-- Gen 37: Gen 30 base + lower push threshold (>=2 advantage instead of >=3).
-- Gen 32 tried pushing at ANY advantage and was slightly worse. This tests the
-- middle ground: push at >=2 advantage. More opportunities to attack buildings
-- without being so aggressive that we push into even fights.
...
local moderate_advantage = my_combat_count >= enemy_count + 2
...
-- === PUSH: >=2 advantage (lower than Gen 30's >=3) ===
local should_push = (enemy_count == 0 and my_combat_count >= 2)
    or moderate_advantage
