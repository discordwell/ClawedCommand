-- @name: combat_micro_commit
-- @events: on_tick
-- @interval: 3

-- Gen 31: Gen 26 base + commit mode.
-- When we have overwhelming advantage (>=5 more army), disable retreat and
-- kiting entirely — all-in push to enemy HQ. This should convert late-game
-- timeouts where P1 has a big lead into decisive wins.
...
local overwhelming = my_combat_count >= enemy_count + 5

-- === COMMIT MODE: overwhelming advantage → all-in, no retreat ===
if overwhelming then
    -- Find enemy HQ and attack-move everything there
    ...
    -- Still focus fire even in commit mode
    if #attackers >= 2 then
        local cx, cy = 0, 0
    ...
    return  -- skip normal retreat/kite logic
end
