-- @name: combat_micro_inline_formation
-- @events: on_tick
-- @interval: 3

-- Gen 53: Gen 34 base + inline formation positioning.
-- Tanks attack_move forward toward enemy, ranged hold position behind centroid.
-- Avoids command conflicts from separate formation script by integrating
-- positioning into the combat micro loop.
-- Only repositions non-attacking idle units to avoid interrupting combat.
...
for _, u in ipairs(my_units) do
    ...
    if not is_worker then
        ...
        if u.attacking then
            ...
            -- Idle combat units get formation positioning
...
-- === INLINE FORMATION: idle tanks forward, idle ranged back ===
if enemies and #enemies > 0 and not outnumbered then
    -- Find nearest enemy to centroid for direction
    ...
    -- Only form up when enemy is at medium range (4-15 tiles)
    if dist >= 4 and dist <= 15 then
        local nx = dx / dist
        local ny = dy / dist

        -- Idle tanks: attack_move 3 tiles toward enemy
        ...
        -- Idle ranged: move 2 tiles behind centroid (away from enemy)
