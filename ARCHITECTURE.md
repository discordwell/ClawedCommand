...
> Isometric 2D RTS where players hybrid-control units directly and through AI agents powered by a fine-tuned Mistral model.

## Vision

Players command armies through a dual interface: traditional RTS point-and-click micro **and** natural language instructions to an AI commander that generates strategy code. The AI agent uses MCP tools to issue game commands, creating a gameplay loop where strategic thinking and AI coaching are as important as mechanical skill.
...
**Entity Types:**
- **Units** — soldiers, vehicles, workers. Components: `Position`, `Health`, `Attack`, `Movement`, `Owner`, `UnitType`, `AIControllable`
- **Buildings** — bases, barracks, refineries, tech labs. Components: `Position`, `Health`, `ProductionQueue`, `Owner`, `BuildingType`
- **Resources** — ore deposits, energy nodes. Components: `Position`, `ResourceType`, `Amount`
- **Projectiles** — bullets, missiles. Components: `Position`, `Velocity`, `Damage`, `Target`
...
enum GameCommand {
    ...
    // Economy commands
    GatherResource { worker_ids: Vec<EntityId>, resource_id: EntityId },
...
**Resources:**
- Primary: Credits (gathered from ore deposits by workers)
- Secondary: Energy (generated by power plants, consumed by advanced units/buildings)
- Supply: population cap increased by supply buildings

**Tech tree structure:**
```
Command Center ──► Barracks ──► Infantry units
       │                  └──► Armory ──► Advanced infantry
       ├──► Refinery ──► Workers (enhanced gathering)
       ├──► Vehicle Factory ──► Vehicles
       │              └──► Tech Lab ──► Advanced vehicles
       ├──► Power Plant ──► Energy production
       └──► Radar Station ──► Extended vision / map reveal
```

(Specifics are game design — this is a starting skeleton to build from)
