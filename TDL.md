...
- [x] `combat_system` invulnerable-target branch (cleared `AttackTarget` only) was the same stale-chase pattern: a unit chasing a target that pops invulnerability kept `ChasingTarget`/`MoveTarget`/`Path`. Not actually self-limiting — invulnerability sources include **Zoomies** (+100% speed), so the target usually flees out of weapon range and the chaser is left permanently "moving" (never idle), exactly like the death case. Fixed by mirroring the death-case handling: (1) `target_acquisition_system` now skips invulnerable candidates when scanning and treats a now-invulnerable current target like a dead one (resets the chaser to idle, same-tick re-scan re-acquires a different in-range enemy), and (2) the `combat_system` branch resets to idle as the in-combat backstop (it remains the guard that actually prevents damage — `damage.rs` does not check `invulnerable`). 3 regression tests in integration.rs (give-up, re-acquire, scan-skip), each confirmed to fail on pre-fix code.
...
- [ ] **Multi-charge abilities can never burst — charges + regen loop are currently inert (balance decision).** On activation, `command_system.rs` (Activated arm, ~L832-835) both re-arms the *full* cooldown (`cooldown_remaining = effective_cooldown`) **and** decrements a charge, while activation is gated on `cooldown_remaining == 0` (L708). `ability_cooldown_system` (ability_system.rs:40-49) refills one charge each time the cooldown hits 0. Net effect: a unit can only fire once per full `cooldown_ticks` regardless of stored charges, so `charges` never drops below `max_charges - 1` and the dedicated regen loop never observably does anything. This makes the burst use-case (spend N charges in quick succession, refill over time) impossible. Affects all 7 abilities with `max_charges >= 2`: DeadDrop(3), BoobyTrap(3), TrinketStash(3), DecoyNest(2), SubterraneanHaul(4), DeepBore(3), FuelReserve(3). Fix direction (if burst is intended): for charge-based abilities, gate activation on `charges > 0` and do **not** re-arm `cooldown_remaining` on use — let the regen timer alone govern refills. This is a balance change (e.g. a Mouser could drop 3 DeadDrops back-to-back), so it needs a design call before implementing.
